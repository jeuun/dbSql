...
CREATE TABLE emp_test(
    ...
    ename VARCHAR(10),
    deptno NUMBER(2) REFERENCES dept_test(deptno) --  dept_test 테이블에 존재하지 않는 데이터만 허용하도록 제약을 걸어줌
);
...
-- 무결성 제약 에러 발생시 뭘 해야 할까 ? -  parent key not found
-- 입력하려고 하는 값이 맞는지 확인하기 : 2번이 맞나? 1번 아냐?
-- 부모 테이블에 값이 왜 입력이 안되었는지 확인 (dept_test 확인) 
...
DROP TABLE emp_test;   -- 자식테이블 먼저 삭제
DROP TABLE dept_test; 

CREATE TABLE dept_test(
    deptno NUMBER(2), -- PRIMARY KEY -> UNIQUE제약X --> 인덱스 생성X
    dname VARCHAR2(14),
    loc VARCHAR(13)
);
...
CREATE TABLE emp_test(
    empno NUMBER(4),
    ename VARCHAR2(10),
    -- dept_test.dept_no컬럼에 인덱스가 없기 때문에 정상적으로 fk 제약을 생성할 수 없다.
...
INSERT INTO dept_test VALUES (1,'ddit','daejeon'); -- 부모
INSERT INTO emp_test VALUES (9999,'brown',1); -- 자식
COMMIT;

-- dept_test테이블의 deptno값을 참조하는 데이터가 있을 경우 삭제가 불가능하다.
-- 자식 테이블에서 참조하는 데이터가 없어야 부모 테이블에서 데이터를 삭제할 수 있다.
DELETE dept_test
WHERE deptno = 1;
...
-- FK 제약 default 옵션시에는 부모 테이블의 데이터를 삭제하기 전에 자식 테이블에서 
-- 참조하는 데이터가 없어야 정상적으로 삭제가 가능했음
-- ON TABLE CASCADE의 경우 부모 테이블 삭제시 참조하는 자식 테이블의 데이터를 같이 삭제한다.
-- 1. 삭제 쿼리가 정상적으로 실행되는지 확인
-- 2. 자식테이블에 데이터가 삭제 되었는지 확인
DELETE dept_test   
WHERE deptno=1;
...
-- FK 제약 default 옵션시에는 부모 테이블의 데이터를 삭제하기 전에 자식 테이블에서 
-- 참조하는 데이터가 없어야 정상적으로 삭제가 가능했음
-- ON TABLE SET NULL의 경우 부모 테이블 삭제시 참조하는 자식 테이블의 데이터의 참조 컬럼을 NULL로 수정한다.
-- 1. 삭제 쿼리가 정상적으로 실행되는지 확인
-- 2. 자식테이블에 데이터가 NULL로 변경 되었는지 확인
DELETE dept_test   
WHERE deptno=1;

SELECT *
FROM emp_test; -- deptno가 NULL로 변경됨
...
-- emp_gb컬럼 체크 제약에 의해 01,02가 아닌 값은 입력될 수 없다.
...
-- CUSTOMER 테이블의 데이터도 같이 복제, primary key는 복제가 되지 않는다. 순수하게 데이터만 가져감
...
-- FOREIGN KEY 추가
-- 실습 
-- 1. DEPT 테이블의 deptno컬럼으로 PRIMARY KEY 제약을 테이블 변경 ddl을 통해 생성
DESC dept;
ALTER TABLE dept ADD CONSTRAINT pk_dept PRIMARY KEY (deptno);

-- 2. emp 테이블의 empno컬럼으로 PRIMARY KEY 제약을 테이블 변경 ddl을 통해 생성
ALTER TABLE emp ADD CONSTRAINT pk_emp PRIMARY KEY (empno);

-- 3. emp 테이블의 deptno컬럼으로 dept테이블의  deptno컬럼을 참조하는 fk제약을 테이블 변경 ddl을 통해 생성
-- emp -> dept (deptno)
